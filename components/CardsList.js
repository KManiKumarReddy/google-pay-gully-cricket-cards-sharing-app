...
export default function CardsList() {
    ...
    useEffect(() => {
        setLoading(true)
        fetch('/api/counts').then((res) => res.json()).then(json => {
            setCardsList(json)
            setLoading(false)
        })
    }, [])
    ...
    return <div className="p-5 gap-5 grid grid-cols-2">
...
function Loader() {
    return <div className="text-center py-5">
        <svg role="status" className="w-16 h-16 inline-block text-gray-200 animate-spin dark:text-gray-200 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function Card({ name, count, imageUrl }) {
    ...
    const claimCard = async () => {
        if (mobileCheck()) {
            ...
            alert('Please open this link via mobile')
        }
    }
    if (loading)
        return <Loader />
